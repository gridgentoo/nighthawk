[Nighthawk]&[envoy]&[Создано с использованием исходников Envoy] генератора нагрузки и инструмента измерения.   

Рассмотрите возможность использования Nighthawk в качестве генератора нагрузки и инструмента измерения.   
Мы стремимся использовать лучшие практики бенчмаркинга и измерения задержек в этом инструменте.   
https://github.com/envoyproxy/nighthawk.  

Генератор нагрузки производительности от сообщества Envoy.  
Nighthawk — это инструмент сравнительного анализа, используемый для тестирования производительности протоколов уровня 7 (HTTP/HTTPS/HTTP2/HTTP3) сетевых уровней.   
Это также может помочь проанализировать производительность уровня связи gRPC.  


[Built using Envoy sources] Создано с использованием исходников Envoy.  
Адаптивное управление нагрузкой -  Adaptive load control.  
несколько выходных форматов - multiple output formats.  

Входные данные:  
[Warmup count] счетчик прогрева,      
[multi-target IP addresses] многоцелевые IP-адреса,   
[HTTP2 load] загрузка HTTP2,   
[open/closed loop] разомкнутый/замкнутый цикл,  
[max requests per connection] максимальное количество запросов на соединение,   
[max active requests] максимальное количество активных запросов,   
[TLS context] контекст TLS,  
[customized headers] настраиваемые заголовки,    
